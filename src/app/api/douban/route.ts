...
async function fetchUrl(url: string, isJson: boolean = true) {
...
export async function GET(request: Request) {
  ...
  const pageSize = pageSizeStr ? Math.min(Math.max(parseInt(pageSizeStr), 1), 100) : 16;
  const pageStart = pageStartStr ? Math.max(parseInt(pageStartStr), 0) : 0;

  if (!type || !tag) {
    return NextResponse.json({ error: 'Missing parameters: type or tag' }, { status: 400 });
  }

  if (!['tv', 'movie'].includes(type)) {
    return NextResponse.json({ error: 'Invalid type: must be tv or movie' }, { status: 400 });
  }
...
async function handleSearchSubjects(type: string, tag: string, pageSize: number, pageStart: number) {
